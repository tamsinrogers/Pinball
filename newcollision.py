...
# updated by Bruce Maxwell for CS 152, Fal 2019
...
def collisionTest_ball_block(ball, block):
    ...
    if tmin < 0: # collision is behind the ball
        # double-check that the ball is not inside the block
        if blockP[0]-dx*0.5 - radius <= ballP[0] <= blockP[0]+dx*0.5+radius and blockP[1]-dy*0.5-radius <= ballP[1] <= blockP[1]+dy*0.5+radius:
            tmin = 0
        else:
            tmin = 1e+6
